...
const checkInvoicesDue = async () => {
  ...
  const due = await Invoice.find({
    ...
    logger.error('Failed to fetch due invoices:', e);
    return [];
  });
  let count = 0;
  for (const inv of due) {
    try {
      ...
      logger.error(`Failed to notify for invoice ${inv.invoiceRef}:`, err);
    }
  }
  if (count > 0) {
    try {
      await audit('Invoice Reminders', `${count} invoice(s) approaching due date.`, ACTIVITY_TYPE.INVOICE);
    } catch (err) {
      logger.error('Failed to audit invoice reminders:', err);
    }
  }
};
...
const checkRfqDeadlines = async () => {
  ...
  const open = await RFQ.find({
    ...
    logger.error('Failed to fetch open RFQs:', e);
    return [];
  });
  for (const rfq of open) {
    try {
      ...
      logger.error(`Failed to notify for RFQ ${rfq.title}:`, err);
    }
  }
};
