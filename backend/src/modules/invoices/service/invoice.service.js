...
const invoiceService = {
  list(query, actor) {
    if (actor?.role === 'vendor' && actor?.vendorId) {
      return invoiceRepository.list({ ...query, vendorId: String(actor.vendorId) });
    }
  ...
  async pay(id, actor) {
    ...
    const updated = await invoiceRepository.updateById(id, { status: INVOICE_STATUS.PAID });
    await events.record({
  ...
  async buildPdf(id) {
    ...
    const buffer = await buildInvoicePdf(inv);
