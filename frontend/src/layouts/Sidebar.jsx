...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside 
        className={`fixed top-0 bottom-0 left-0 z-45 w-64 bg-black/80 backdrop-blur-md border-r border-zinc-900/80 flex flex-col transition-transform duration-300 lg:translate-x-0
          ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Brand/Logo Header */}
        <div className="h-16 border-b border-zinc-900/60 px-5 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 rounded-lg bg-violet-500/10 border border-violet-500/30 flex items-center justify-center glow-violet">
              <span className="text-violet-400 font-bold text-base tracking-wider group-hover:scale-105 transition-transform">VB</span>
            </div>
            <span className="text-zinc-100 font-bold text-md tracking-wider group-hover:text-violet-400 transition-colors">
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) => `
                  ...
                  ${isActive 
                    ? 'bg-violet-500/8 border border-violet-500/20 text-violet-400 font-medium shadow-[0_0_15px_rgba(139,92,246,0.06)]' 
              ...
              >
                {({ isActive }) => (
                  <>
                    <Icon className={`w-4.5 h-4.5 transition-transform group-hover:scale-105 ${isActive ? 'text-violet-400' : 'text-zinc-500 group-hover:text-zinc-300'}`} />
                    <span>{item.name}</span>
                  </>
                )}
              </NavLink>
