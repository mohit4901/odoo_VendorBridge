...
const Vendors = () => {
  ...
  const handleAddVendor = async (e) => {
    e.preventDefault();
    try {
      const res = await vendorService.create(formData);
      if (res.success) {
      ...
      console.error(err);
      addNotification(err.message || 'Failed to add vendor', 'error');
    }
  };
  ...
  const handleEditVendor = async (e) => {
    e.preventDefault();
    try {
      const res = await vendorService.update(selectedVendor._id || selectedVendor.id, formData);
      if (res.success) {
        await refreshVendors();
      ...
      console.error(err);
      addNotification(err.message || 'Failed to update vendor', 'error');
    }
  };
  ...
  const handleDeleteVendor = async () => {
    try {
      const res = await vendorService.remove(selectedVendor._id || selectedVendor.id);
      if (res.success) {
        await refreshVendors();
      ...
      console.error(err);
      addNotification(err.message || 'Failed to delete vendor', 'error');
    }
  };
